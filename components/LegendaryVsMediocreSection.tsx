...
// Helper component for the hexagonal cycle diagram
const CycleDiagram: React.FC<{ variant: 'legendary' | 'mediocre' }> = ({ variant }) => {
  const isLegendary = variant === 'legendary';
  const color = isLegendary ? '#C9B298' : '#666'; // Gold vs Gray
  const arrowColor = isLegendary ? '#fff' : '#444'; 
  
  // Data Configuration
  const data = isLegendary 
    ? {
        title: "VIDA LENDÁRIA",
  ...
  return (
    <div className="relative aspect-square md:aspect-[4/3] w-full max-w-md mx-auto select-none">
       {/* Title */}
       <div className="text-center mb-8">
           <h4 className={`font-sans font-bold text-xl tracking-widest ${isLegendary ? 'text-white' : 'text-zinc-400'}`}>
               {data.title}
           </h4>
       </div>
       ...
       <div className="relative w-full pb-[100%]">
          <svg className="absolute inset-0 w-full h-full" viewBox="0 0 100 100" overflow="visible">
             ...
             <g fill="none" stroke={arrowColor} strokeWidth="0.5" markerEnd={`url(#arrow-${variant})`}>
                 {/* Top to Top-Right */}
             ...
             {/* Central Triangle */}
             <g stroke={isLegendary ? '#C9B298' : '#555'} strokeWidth="0.5" fill="none">
          ...
          {data.nodes.map((node, i) => (
              <div 
                key={i}
                className={`absolute font-serif text-[10px] md:text-xs font-bold leading-tight
                    ${isLegendary ? 'text-white' : 'text-zinc-500'}
          ...
          {data.triangle.map((node, i) => (
              <div 
                key={`tri-${i}`}
                className={`absolute font-sans text-[8px] md:text-[9px] font-medium leading-tight whitespace-pre
                    ${isLegendary ? 'text-[#C9B298]' : 'text-zinc-600'}
          ...
          {/* Center Label */}
          <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 font-sans font-bold text-xs ${isLegendary ? 'text-white' : 'text-zinc-700 decoration-zinc-500 line-through'}`}>
             {data.center}
          </div>
       </div>
       ...
       <div className="text-center mt-4">
           <h5 className={`font-serif italic text-lg ${isLegendary ? 'text-white' : 'text-zinc-500'}`}>
               {data.subtitle}
           </h5>
       </div>
    </div>
    ...
    <div className="space-y-20 animate-fade-in">
      ...
      <section className="bg-[#0A0A0A] rounded-3xl p-8 md:p-12 shadow-2xl border border-white/5 overflow-hidden">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-8 items-center relative">
            ...
            <CycleDiagram variant="legendary" />
            <CycleDiagram variant="mediocre" />
        </div>
        ...
        <div className="relative rounded-3xl overflow-hidden bg-foreground text-background p-8 md:p-16">
            ...
            <div className="relative z-10 grid grid-cols-1 xl:grid-cols-2 gap-12 items-center">
                ...
                <div className="relative rounded-xl overflow-hidden border-4 border-destructive/20 shadow-2xl bg-[#0A0A0A] p-4">
                     {/* Using the cycle diagram component inside the card for visual consistency */}
                     <CycleDiagram variant="mediocre" />
                </div>
            </div>
        </div>
